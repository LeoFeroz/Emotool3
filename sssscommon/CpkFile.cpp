...
bool CpkFile::ParseTocData(uint8_t *buf)
{
	...
	{
		temp.dir_name = (char *)GetColumnData(toc, i, "DirName");
		temp.file_name = (char *)GetColumnData(toc, i, "FileName");
		temp.file_size = GetColumnDataAsInteger(toc, i, "FileSize");
		temp.extract_size = GetColumnDataAsInteger(toc, i, "ExtractSize");
		temp.file_offset = GetColumnDataAsInteger(toc, i, "FileOffset") + add_offset;
		temp.dir_name_pos = GetColumnPosition(toc, i, "DirName");
		temp.file_name_pos = GetColumnPosition(toc, i, "FileName");
