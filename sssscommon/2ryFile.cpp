...
void PtcEntry::Decompile(TiXmlNode *root, const ScdEntry &scd) const
{
    ...
    Utils::WriteParamMultipleFloats(entry_root, "F_10", std::vector<float>(unk_10, unk_10+6));
...
bool PtcEntry::Compile(const TiXmlElement *root, ScdEntry &scd)
{
    ...
    if (!Utils::GetParamMultipleFloats(root, "F_10", this->unk_10, 6))
        return false;
...
bool _2ryFile::Load(const uint8_t *buf, unsigned int size)
{
    ...
    for (size_t i = 0; i < scds.size(); i++)
    {
        ...
        for (size_t j = 0; j < scd.ptcs.size(); j++)
        {
            ...
            if (ptcs[j].unk_28[0] != 0 || ptcs[j].unk_28[1] != 0)
            {
                DPRINTF("%s: unk_28 not 0 as expected, in SCD 0x%x, in PTC 0x%x\n", FUNCNAME, i, j);
                return false;
            }
...
uint8_t *_2ryFile::CreateFile(unsigned int *psize)
{
    ...
    for (size_t i = 0; i < scds.size(); i++)
    {
        ...
        shdr->unk_08 = val_float(scd.unk_08);
        shdr->unk_0C = val_float(scd.unk_0C);
        ...
        for (size_t j = 0; j < scd.ptcs.size(); j++)
        {
            ...
            for (int k = 0; k < 6; k++)
            {
                ptcs[j].unk_10[k] = val_float(ptc.unk_10[k]);
            }
        }
        ...
        for (size_t j = 0; j < scd.jnts.size(); j++)
        {
            ...
            for (int k = 0; k < 5; k++)
            {
                jnts[j].unk_0C[k] = val_float(jnt.unk_0C[k]);
            }
        }
        ...
        for (size_t j = 0; j < scd.cols.size(); j++)
        {
            ...
            for (int k = 0; k < 8; k++)
            {
                cols[j].unk_94[k] = val_float(col.unk_94[k]);
            }
        }
    }
