...
EmbFile::EmbFile(uint8_t *buf, unsigned int size, bool recursive)
{
    this->recursive = recursive;
    Load(buf, size);
}
...
bool EmbFile::Load(const uint8_t *buf, unsigned int size)
{
    Reset();
...
bool EmbFile::ReplaceFile(uint16_t idx, uint8_t *buf, size_t size)
{
    if (idx >= files.size())
        return false;
...
bool EmbFile::ReplaceFile(const std::string &name, uint8_t *buf, size_t size)
{
...
uint16_t EmbFile::AppendFile(uint8_t *buf, size_t size, const std::string &name)
{
    EmbContainedFile file;
    uint16_t idx = files.size();
...
unsigned int EmbFile::CalculateFileSize()
{
    ...
    file_size += files.size() * sizeof(EMBTable);
    if (HasFileNames())
    {
        file_size += files.size() * sizeof(uint32_t);
    }
...
uint8_t *EmbFile::CreateFile(unsigned int *psize)
{
    uint8_t *ret;
    ...
    for (size_t i = 0; i < files.size(); i++)
    {
        ...
        table[i].file_size = val32(files[i].size);
