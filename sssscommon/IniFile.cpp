...
{	
	...
	size1 = this->data_size - (this->ptr_current - this->data);
	len = strlen(line);
	size2 = len + 2;
...
uint8_t *IniFile::CreateFile(unsigned int *size)
{
...
bool IniFile::Load(const uint8_t *buf, unsigned int size)
{	
	Reset();
...
bool IniFile::FindSection(const char *name)
{
	...
	while ((line = this->ReadLine()))
	{
		...
		if (this->IsSection(line))
		{
			...
			for (int i = strlen(comp_start)-2; i >= 0; i--)
			{
			...
			compare_size = (comp_end - comp_start);
...
bool IniFile::SetStringValue(const char *section, const char *name, const char *value, bool must_exist, bool with_quotes)
{
	...
	len = n_special_chars + strlen(value) + ((with_quotes) ? 2 : 0);
