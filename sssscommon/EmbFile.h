...
public:
    ...
    EmbFile(uint8_t *buf, unsigned int size, bool recursive=false);
    ...
    bool ReplaceFile(uint16_t idx, uint8_t *buf, size_t size);
    bool ReplaceFile(const std::string &name, uint8_t *buf, size_t size);

    uint16_t AppendFile(uint8_t *buf, size_t size, const std::string &name);
