...
ChcFile::ChcFile(uint8_t *buf, unsigned int size)
{
	this->big_endian = false;
	Load(buf, size);
}
...
bool ChcFile::Load(const uint8_t *buf, unsigned int size)
{
	Reset();
...
uint8_t *ChcFile::CreateFile(unsigned int *psize)
{
