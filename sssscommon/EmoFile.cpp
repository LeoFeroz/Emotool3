...
EmoFile::EmoFile(uint8_t *buf, unsigned int size)
{
    Load(buf, size);
}
...
bool EmoFile::Load(const uint8_t *buf, unsigned int size)
{
    Reset();
...
uint8_t *EmoFile::CreateFile(unsigned int *psize)
{
    unsigned int file_size;
    ...
    uint8_t *skl = SkeletonFile::CreateFile(&skl_size);
    if (!skl)
    {
        delete[] buf;
        return nullptr;
