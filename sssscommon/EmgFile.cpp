...
bool SubPart::InjectVertex(const std::vector<VertexCommon> &new_vertex, bool do_pos, bool do_uvmap, bool do_normal)
{
    ...
    {
        DPRINTF("ERROR: When injecting, number of vertex must be the same than existing ones! (tried: %Iu, existing: %u), on subpart \"%s\"\n", new_vertex.size(), vertex.size(), meta_name.c_str());
        return false;
    }
...
void SubPart::ExportSubMeshFbxSkin(const SkeletonFile &skl, const SubMesh &submesh, const std::vector<VertexData> &sm_vertex, const std::vector<FbxNode *> &fbx_bones, FbxScene *scene, FbxMesh *fbx_mesh, FbxAMatrix skin_matrix) const
{
    ...
    // Weights are stored with only 3 explicit floats per vertex (blend_weight[0..2]);
    // the 4th influence is implicit ("1.0 - sum of the other three"), which is how the
    // original engine packs them. The old code trusted that arithmetic blindly: any
    // rounding/quantization noise coming from the source file that made the first three
    // weights sum to >= 1.0 turned the implicit 4th weight negative, and it was then
    // silently dropped (`this_weight > 0.0f` check) with no renormalization of what was
    // left. The vertex ended up with a weight sum < 1.0, which is exactly what causes
    // wrong/partial deformation when the FBX is opened in Blender/Maya (the "missing"
    // weight is implicitly treated as no-influence instead of being redistributed).
    // Fix: clamp negative/near-zero noise instead of discarding it, then renormalize
    // the 4 weights of each vertex so they always sum to exactly 1.0 before handing
    // them to the FBX skin clusters.
    const float WEIGHT_EPSILON = 1e-5f;
    ...
    for (size_t i = 0; i < sm_vertex.size(); i++)
    {
        ...
        float weights[4];
        weights[0] = blend_weight[0];
        weights[1] = blend_weight[1];
        weights[2] = blend_weight[2];
        weights[3] = 1.0f - (weights[0] + weights[1] + weights[2]);

        // Clamp floating point noise instead of throwing the influence away.
        for (int k = 0; k < 4; k++)
        {
            if (weights[k] < 0.0f)
                weights[k] = 0.0f;
        ...
        // Renormalize so every vertex always ends up fully weighted (sum == 1.0),
        // regardless of any precision loss in the source data.
        if (weight_sum > WEIGHT_EPSILON && fabsf(weight_sum - 1.0f) > WEIGHT_EPSILON)
        {
            for (int k = 0; k < 4; k++)
        ...
        {
            float this_weight = weights[j];

            if (this_weight > WEIGHT_EPSILON)
            {
                ...
                assert(bone->GetName() == skin_clusters[linked_bone_index]->GetName());
...
bool EmgFile::Load(uint8_t *buf, unsigned int size, SkeletonFile *skl)
{
    UNUSED(size);
