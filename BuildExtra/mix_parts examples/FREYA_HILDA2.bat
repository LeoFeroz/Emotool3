...
::
:: This bat file generates "Evil Freya 2", alias "FREYA_HILDA2"
::
...
::
:: The base here is Hilda, because it contains everything except the face/hair.
set base="HLD_00"
...
::
:: The other here is Freya, from which will take only the hair and face.
:: (and actually, only the upper part of the face, since the other part is identical to Hilda except in lips color,
::  and we want Hilda lips color)
set other="FRA_00"
...
::
:: FREYA_HILDA2 is the codename of Evil Freya 2.
set result="FREYA_HILDA2"
...
::
:: In this case, we don't want Hilda face/hair.
:: We are removing the crown too, because it doesn't look well with Freya accesory.
:: (and we can't remove Freya accesory, because the hair below is not modelled, sadly).
set parts_to_remove="PARTS_hair,EDGE_PARTS_hair,PARTS_hair_back,EDGE_PARTS_hair_back,face,edge,CLOTH_crawn,EDGE_CLOTH_crawn"

:: These are the parts of "other" that we want in the final model.
::
:: In this case, we want the Freya face, hair and hair accesory.
set parts_to_add="PARTS_hair,EDGE_PARTS_hair,PARTS_hair2,EDGE_PARTS_hair2,PARTS_hair3,EDGE_PARTS_hair3,PARTS_head_acc,EDGE_PARTS_head_acc,face,edge"
...
::
:: No need to use this at all for Evil Freya 2.
...
::
:: Evil Freya 2 doesn't need this shit.
