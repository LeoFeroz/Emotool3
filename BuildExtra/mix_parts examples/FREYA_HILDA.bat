...
::
:: This bat file generates "Evil Freya", alias "FREYA_HILDA"
::
...
::
:: The base here is obviously Freya, as we are only appending a weapon and a ring to her.
set base="FRA_00"
...
::
:: The other here is Hilda, from which we will take the weapon and the ring.
set other="HLD_00"
...
::
:: FREYA_HILDA is the codename of Evil Freya.
set result="FREYA_HILDA"
...
::
:: In this case, since we don't want to remove anything from Freya, just use NULL
set parts_to_remove="NULL"

:: These are the parts of "other" that we want in the final model.
::
:: In this case, we want the weapon and the ring.
set parts_to_add="weapon,EDGE_weapon,CLOTH_ring,EDGE_CLOTH_ring"
...
::
:: Evil Freya is simple enough to not need this param.
...
::
:: No need to use this at all for Evil Freya (Evil Freya is not a "replace part" mod, but only an "add part" mod)
...
::
:: Evil Freya doesn't need this shit.
