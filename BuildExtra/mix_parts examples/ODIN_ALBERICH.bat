...
::
:: This bat file generates "Odin Alberich"
::
...
::
:: The base here is Odin Seiya, which contains most things
set base="ODN_00"
...
::
:: The other here is Alberich, obviously.
set other="ALB_00"
...
::
:: In this case, we don't want Seiya face/hair/neck (note that neck includes lower part of face)
:: We don't want fingers from Seiya either, because skin color of Alberich is different to Seiya.
:: (although that can be done with texturing too, but we will do it directly in the model)
set parts_to_remove="PARTS_hair,EDGE_PARTS_hair,face,edge,PARTS_neck,EDGE_PARTS_neck,SHARE_finger,EDGE_SHARE_finger"

:: These are the parts of "other" that we want in the final model.
::
:: We pretty much want from Alberich what we removed from Seiya. Parts had similar names in this case, 
:: and Seiya/Alberich hairs only have one component. But things are not always like that, keep that in mind.
set parts_to_add="PARTS_hair,EDGE_PARTS_hair,face,edge,PARTS_neck,EDGE_PARTS_neck,SHARE_finger,EDGE_SHARE_finger"
...
::
:: We don't want Head/Neck/HEN bones to be added, since it would cause lot of trouble.
set bones_to_not_add="Head,Neck,HEN"
...
::
:: We don't want the bones of neck, finger and their respective edges.
set exclude_bones_parts="PARTS_neck,EDGE_PARTS_neck,SHARE_finger,EDGE_SHARE_finger"
...
::
:: Odin Alberich is complex enough to need this, and the GUESS algorithm works with it.
:: The GUESS algorithm however made us to add "HEN" to bones_to_not_add, because the algorithm was
:: picking it up.
