...
::
:: This bat file generates "Saga god armor without helmet" evil version
::
...
::
:: Just a name.
set result="SAGA_GOD_ARMOR_WITHOUT_HELMET_EVIL"
...
::
:: In this case, we don't want the helmet and the hair of Saga God armor, and we don't want the upper fart of face (that contains the eye)
set parts_to_remove="PARTS_hair,EDGE_PARTS_hair,CLOTH_helmet,EDGE_CLOTH_helmet,face,edge"

:: These are the parts of "other" that we want in the final model.
::
:: In this case, we want the hair from the other Saga, and also upper part of face for the eyes
set parts_to_add="PARTS_hair,EDGE_PARTS_hair,face,edge"
